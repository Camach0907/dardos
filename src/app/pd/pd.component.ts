...
export class PdComponent implements OnInit {
  ...
  //variaveis para escolher a pontuação e mostrar as tabelas
  public show : boolean = false;
  public choosenScore : number = 0;
  ...
  //Var que agarra o elemento do pop-up
  public popUpElement : any = null;
  ...
  Score({event}: { event: any }){
    ...
    //Verifica se escolheu o Nr de Rondas, caso não tenha escolhido apresenta um Warning
    if(this.round > 0)
    {
      //Agarra o Score
      this.choosenScore = event.target.id == "1" ? 301 : event.target.id == "2" ? 501 : 701;

      //Cria os arrays dos jogadores
      for(let i = 0; i<this.playerCount;i++){
        this.throw.push({
          play: 0,
          currentScore: this.choosenScore,
          total: 0,
          SavedScore : 0,
          darts : [0,0,0]
        });
  ...
  toggle({event}: { event: any }) {
    ...
    //Se estivermos na ultima jogada do ultimo jogador --> voltamos ao primeiro jogador e ao primeiro dardo e grava-se o score para quando rebenta
  ...
  public Bull() : void{
    ...
    if (this.throw[this.playerId].currentScore < 0) {

      this.Boom();

      //Caso ganhe, renicia as variaveis para recomeçar o jogo
    } else if (this.throw[this.playerId].currentScore == 0) {
...
// ---------------------------------------------------------------------------------------------------------------------
  ...
  public removeDarts2(): void{
    ...
    //this.removedarts = true;

    /*this.notify.info( 'Remove os dardos!', {
      position: SnotifyPosition.centerCenter,
      showProgressBar: false,
      closeOnClick: true,
      timeout: 0,
      buttons: [
        {text: 'OK', action: () => this.ClearBoard2(), bold: true},
      ]
    });*/
  ...
  public ClearBoard(): void{
    ...
    //Grava o score
    this.throw[this.playerId].SavedScore = this.throw[this.playerId].currentScore;
  ...
  public ClearBoard2(): void{
    ...
    //Grava o score
    this.throw[this.playerCount-1].SavedScore = this.throw[this.playerCount-1].currentScore;
  ...
  public EndGame(): void{
    ...
    if(confirm('O jogador '+(this.showPlayerId)+' GANHOU!! \n\n Ok -> jogar novamente || Cancel -> voltar ao início.')){
      this.playerId = 0;
      this.showPlayerPlay = 1;
      this.showPlayerId = 1;

      for (let i = 0; i < this.playerCount; i++) {
        this.throw[i].currentScore = this.choosenScore;
  ...
  //Quando o jogador rebenta
  public Boom(): void{


    alert('Player '+ (this.showPlayerId) +' REBENTOU!');

    //Limpa
    this.throw[this.playerId].currentScore = this.throw[this.playerId].SavedScore;
    this.throw[this.playerId].play = 0;
    this.throw[this.playerId].total = 0;
    this.throw[this.playerId].darts[0] = 0;
    this.throw[this.playerId].darts[1] = 0;
    ...
    if(this.playerId < this.playerCount-1) {
      //Atualiza
      this.throw[this.playerId].play = 0;
      this.showPlayerPlay = 1;
      this.playerId ++;
      ...
      this.showPlayerId = 1;
      //Se estivermos na última ronda e for o último player a jogar
      if(this.round == this.roundCounter){this.Reset();}
